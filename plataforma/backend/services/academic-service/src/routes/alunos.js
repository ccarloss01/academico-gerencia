...
router.get('/', requireAuth, async (_req, res) => {
  const result = await client.execute('SELECT * FROM alunos ORDER BY nome');
...
router.post('/', requireAuth, async (req, res) => {
  ...
  try {
    const result = await client.execute({
      sql: 'INSERT INTO alunos (user_id, nome, matricula, curso) VALUES (?,?,?,?) RETURNING *',
      args: [user_id, nome, matricula, curso],
    });
    res.status(201).json(result.rows[0]);
  } catch (err) {
    if (err.message?.includes('UNIQUE constraint failed')) return res.status(409).json({ error: 'Aluno já cadastrado' });
...
router.get('/:id/matriculas', requireAuth, async (req, res) => {
  const result = await client.execute({
    sql: `SELECT m.*, t.semestre, t.horario, d.nome AS disciplina_nome
          FROM matriculas m
